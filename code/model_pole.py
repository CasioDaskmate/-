...
while True:
    ...
    center_x = width // 2
    center_y = height // 2
    rotation_matrix = cv.getRotationMatrix2D((center_x, center_y), ellipses_dict[0][2] - 90, 1)
    # 执行旋转变换
    #cv.ellipse(r_frame, (int(ellipses_dict[0][0][0]),int(ellipses_dict[0][0][1])), (300, 300), 0, ellipses_dict[0][0][0] - 220, ellipses_dict[0][0][0] +30, (0, 255, 255), 2)
    #cv.line(r_frame, (int(ellipses_dict[0][0][0]),int(ellipses_dict[0][0][1])), (int(ellipses_dict[0][0][0] + 300 * math.cos(math.radians(ellipses_dict[0][2] - 90))),int(ellipses_dict[0][0][1] + 300 * math.sin(math.radians(ellipses_dict[0][2] - 90)))), (255, 255, 255), 2, cv.LINE_AA)
    #cv.line(r_frame, (int(ellipses_dict[0][0][0]),int(ellipses_dict[0][0][1])), (int(ellipses_dict[0][0][0] + 300 * math.cos(math.radians(ellipses_dict[0][2] - 210))),int(ellipses_dict[0][0][1] + 300 * math.sin(math.radians(ellipses_dict[0][2] - 210)))), (0, 255, 255), 1, cv.LINE_AA)
    #cv.line(r_frame, (int(ellipses_dict[0][0][0]),int(ellipses_dict[0][0][1])), (int(ellipses_dict[0][0][0] + 300 * math.cos(math.radians(ellipses_dict[0][2] + 30))),int(ellipses_dict[0][0][1] + 300 * math.sin(math.radians(ellipses_dict[0][2] + 30)))), (0, 255, 255), 1, cv.LINE_AA)
    height, width = r_frame.shape[:2]
    rotated_image = cv.warpAffine(r_frame, rotation_matrix, (width, height))
